...
func TestParseReposMinimal(t *testing.T) {
	...
	r, err := ParseReposLines(csv, 1, "https://gitlab.com")
	if err != nil {
		t.Fatal(err)
	}
	if len(r) != 2 {
	...
	if r[0] != (RepoEntry{ID: "1", Path: "g/a", Nest: 1}) {
		t.Fatalf("r[0] = %+v", r[0])
	}
	if r[1] != (RepoEntry{ID: "2", Path: "g/b", Nest: 3}) {
		t.Fatalf("r[1] = %+v", r[1])
	}
}
...
func TestParseReposMismatchedBaseErrors(t *testing.T) {
	if _, err := ParseReposLines("1,https://other.com/g/a\n", 1, "https://gitlab.com"); err == nil {
		t.Fatal("expected error")
	}
}
func TestParseReposTrimsWhitespace(t *testing.T) {
	...
	if r[0] != (RepoEntry{ID: "1", Path: "g/a", Nest: 2}) {
		t.Fatalf("r[0] = %+v", r[0])
	}
}
