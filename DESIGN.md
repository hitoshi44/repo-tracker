...
- `id` 列は GitLab project id (sh 出力の現状列を温存するためのパススルー、fetcher では使わない)
- `url` を `gitlab.url` の prefix で剥がして `group/repo` に変換 → 既存の path ベース API 呼び出しに渡す
- `nest` 省略時は `defaults.nest`
- 起動: `cargo run -- path/to/config.yml` (省略時は cwd の `config.yml`)
